...
// Helper to strip correct answers for student API delivery and shuffle order deterministically per student
export const getStudentRound1Questions = (seed) => {
  const list = ROUND1_QUESTIONS_BACKEND.map(({ id, topic, question, options }) => ({
    id,
    topic,
    question,
    options
  ...
  // Seeded PRNG (Mulberry32) for deterministic per-student question order
  let hash = 2166136261;
  const strSeed = String(seed);
  for (let i = 0; i < strSeed.length; i++) {
    hash = Math.imul(hash ^ strSeed.charCodeAt(i), 16777619);
  }
